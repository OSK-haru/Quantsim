...
class RustLindbladRhsKernelTest(unittest.TestCase):
    def test_zero_h_and_zero_collapse_ops_returns_zero_matrix(self) -> None:
        ...
        result = yuragi_strider_rust.lindblad_rhs_flat(
            _flatten(rho),
            _flatten(zero),
            [],
            0,
    ...
    def test_invalid_inputs_raise_value_error(self) -> None:
        ...
        with self.assertRaises(ValueError):
            yuragi_strider_rust.lindblad_rhs_flat([1.0, 0.0], valid, [], 0, 2)

        with self.assertRaises(ValueError):
            yuragi_strider_rust.lindblad_rhs_flat(valid, [1.0, 0.0], [], 0, 2)

        with self.assertRaises(ValueError):
            yuragi_strider_rust.lindblad_rhs_flat(valid, valid, [1.0, 0.0], 1, 2)

        with self.assertRaises(ValueError):
            yuragi_strider_rust.lindblad_rhs_flat([], [], [], 0, 0)
