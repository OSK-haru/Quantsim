"""Prepare and run a denser Ramsey follow-up with same-job readout calibration."""
...
CONFIRMATION = "RUN_PHASE3B_RAMSEY_DENSE_FOLLOWUP"
SHOTS = 256
...
def main() -> int:
    ...
    plan = [("readout_zero", 0), ("readout_one", 0)] + [
        ("ramsey", delay_dt) for delay_dt in DELAY_GRID_DT
    ]
    records = []
    circuits = []
    ...
    summary = {
        "followup_id": "phase3b_ramsey_dense_followup_v1",
        "backend": metadata,
        "circuits": len(circuits),
        "shots_per_circuit": SHOTS,
        "total_shots": len(circuits) * SHOTS,
        "ramsey_points": len(DELAY_GRID_DT),
    ...
    path = args.output_dir / f"phase3b_ramsey_dense_followup_{job_id}.json"
