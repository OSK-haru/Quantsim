...
run result. See `docs/physics/gate-aware-advanced-gate-compilation.md`.
...
`MEASURE` is implemented as a non-selective computational-basis projective
channel at its circuit-column boundary. Its outcome is currently discarded;
classical conditioning and readout error are not implemented. Independently,
...
Uhlmann fidelity. See `docs/physics/gate-aware-measurement-model.md` for the
formula, implementation transformation, and current limitations.
...
simulator. See `docs/physics/pulse-baseline-a-model.md`,
`docs/physics/pulse-extension-b-qutrit-contract.md`, and
`docs/physics/pulse-coupled-transmon-pair-model.md`.
