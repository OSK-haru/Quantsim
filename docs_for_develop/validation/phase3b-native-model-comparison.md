...
approximately `6.7 kHz` oscillation while the local comparison has no detuning
...
angular frequency. The scan used `0` to `10 kHz` in `0.1 kHz` increments.

| Diagnostic | Value |
|---|---:|
| Zero-detuning Ramsey RMSE | 0.374 |
| Best detuning | 6.7 kHz |
