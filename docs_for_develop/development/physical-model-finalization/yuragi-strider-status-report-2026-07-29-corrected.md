...
Phase 3A: Independent QuTiP audit             COMPLETE
Phase 3B: Hardware observable audit           IN PROGRESS
Phase 4: Final model decision/documentation   PLANNED
```

Phase 3Aでは、frozen CPTP pathとQuTiPの直接比較が完了した。Phase 3Bでは
`QHAD-v1`を正式holdout用datasetとして選定し、dataset contractをfreezeした。
hardware collectionはまだ開始していない。

gate-aware explicit CPTPは追加実装と独立QuTiP監査まで完了した。
Pulse Sequence Editor、Pulse専用State Explorer、circuit-to-pulse compilationは
引き続き有力な将来候補だが、現在の物理モデル最終化計画の次工程として確定して
はいない。
...
| QuTiP監査 | 全体を未完了と読める | 既存RK4比較とfrozen CPTP比較が完了 |
...
```text
1. Gate-aware CPTP実装と監査記録のfreeze
2. Pilot費用・shot・job・timeout・retry上限のfreeze
3. 回路・delay grid・bit order・transpile方針のfreeze
4. Provider-neutral raw-data manifestの作成
5. 非formal pilotの実行
6. Formal calibration / holdout hardware audit
7. Model discrepancyの分類
...
- `QHAD-v1`のpilotとformal holdoutを分離
- calibration caseとholdout caseを事前固定
- raw counts、compiled circuit、job ID、同時刻calibrationを保存
- 費用、shot、job、timeout、retry上限を実行前に固定
- Gate-aware evolution methodの未解決条件を解消
- Toleranceをformal holdout確認前に固定
- 失敗ケースも保存
- 数値一致と実機妥当性を混同しない
...
**2026-07-29 update:** Implemented. See
[`gate-aware-explicit-cptp.md`](gate-aware-explicit-cptp.md). The implementation
preserves the gate-aware Hamiltonian-Lindblad model and adds selectable
constant-GKSL exponential maps with Choi audits. `MEASURE` remains an identity
placeholder and is not claimed as a projective measurement.
...
Frozen CPTP-to-QuTiP audit            COMPLETE
Hardware observable audit            IN PROGRESS
Final physical-model decision         NOT STARTED
Final public documentation            NOT STARTED

Gate-aware explicit CPTP              COMPLETE
...
- 任意の量子デバイスに対する予測精度
- `MEASURE`を射影測定として扱うGate-aware回路全体のCPTP instrument
- Multi-qubit Pulse control
- Non-Markovian dynamics
- Laboratory-frame carrier simulation

次の正式作業は、gate-aware explicit CPTPのfreeze記録を確定し、その後に
pilot実行予算と回路contractをfreezeすることである。
