...
Choi行列は $(2^n)^2 \times (2^n)^2$ のサイズを持つため、量子ビット数に対して急速に増大します。密度行列経路そのものは8量子ビットまで実行できますが、**明示的CPTPが使える範囲はそれより狭い**点に注意してください。

| 条件 | 挙動 |
|---|---|
| ノイズあり・6量子ビット以上でCPTPを要求 | **拒否**(`UNSUPPORTED_EVOLUTION_METHOD`)。RK4を選ぶ必要があります |
| 5量子ビット以上の**条件付き回路**でCPTPを要求 | **RK4へ強制フォールバック**(拒否ではない) |

1行目はブロッキングエラーで、シミュレーションは実行されません。2行目はフォールバックで、結果は返りますが発展方式が変わります。

ノイズを含む回路では1行目が先に効くため、フォールバックが観測されるのは 5 量子ビットのときだけです。理想環境の条件付き回路(測定を含むため密度行列経路を通ります)では、5〜8量子ビットのいずれでもフォールバックが起こります。
...
:::note[メッセージの文言について]
実装のメッセージは「5量子ビット」と書かれていますが、判定条件は **5量子ビット以上**です。理想環境の6〜8量子ビット条件付き回路でも同じ文言が返ります。実際の量子ビット数はリクエストから確認してください。
:::

主軌跡と分岐実行の判定は独立しています。分岐だけがRK4に制限され、主軌跡はCPTPを維持する場合があります。フォールバックは診断 `evolution_method_fallback` と警告としてクライアントに通知されます。

Coupled transmon networkモデルでは、CPTP区間数の上限が 500 に設定されています。CPTP経路が選べるのはHilbert次元が9以下(3準位×最大2台、2準位×最大3台)のときだけで、それを超える構成は固定ステップRK4のみになります。
