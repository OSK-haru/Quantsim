...
4. 🔬 **回路スタジオ**でq0に H を1つ置き、**シミュレーションワークスペース**の「総シミュレーション時間 [μs]」を回路時間より十分長くして実行せよ。画面下の「計算条件の詳細」を開き、「占有数緩和レート」$\Gamma_1$ と「純位相緩和レート」$\gamma_\phi$ から $T_2=1/(\Gamma_1/2+\gamma_\phi)$ を求める。**状態エクスプローラー**の「指標タイムライン」で純度を読み、$P(t)=(1+e^{-2t/T_2})/2$ と比較せよ。

5. 🔬 同じ結果を**状態エクスプローラー**で開き、「Bloch球」と「密度行列」で「理想状態(ノイズなし)」側とGate-aware側を見比べよ。理想側はBloch球の表面に留まる一方、Gate-aware側だけが内側へ縮むことを確かめる。理想側はアプリが同じ回路から自動生成するため、追加の設定は不要である。この差がGKSL方程式の散逸項を除いた場合に対応することを説明せよ。
