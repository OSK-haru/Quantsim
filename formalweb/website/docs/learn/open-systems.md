...
5. 🔬 同じ結果を**状態エクスプローラー**で開き、「指標タイムライン」の理想(ノイズなし)の線とGate-awareの線を比較せよ。理想線では純度が1のまま保たれ、Gate-aware側だけが低下することを確かめる。理想線はアプリが同じ回路から自動生成するため、追加の設定は不要である。この差がGKSL方程式の散逸項を除いた場合に対応することを説明せよ。
