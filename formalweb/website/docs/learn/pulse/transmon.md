...
5. 🔬 **Pulse 回路スタジオ**で同じGaussian・回転角 $\pi$ のX Pulseを保存する。**Pulseラボ**で「準位数」を「3準位 qutrit(漏れ準位あり)」にし、「非調和性 [MHz]」だけを `-250` と `-150` に変えて実行せよ。「最大リーケージ P2」「最終リーケージ P2」を比較し、絶対値の小さい非調和性で漏れが増えるか確認せよ。
