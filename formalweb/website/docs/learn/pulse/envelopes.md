...
6. 🔬 Pulse Lab の Baseline A で、同じ目標回転角 $\pi$ を `square` と `gaussian` の両方で実行し、終状態が(2準位なので)一致することを確かめよ。

7. 🔬 同じ2つを Extension B で実行し、今度は**一致しない**ことを確かめよ。差はどちらの形状に不利に出るか。P2.2 の議論と整合するか。
