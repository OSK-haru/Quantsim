...
6. 🔬 Extension B を `direct_rates` モードで実行し、$\gamma_{21}$ だけを $\gamma_{10}$ と同じ値に変えてみよ。漏れた population の減衰がどう変わるか。

7. 🔬 $\gamma_{\phi,\text{adj}}$ を有限にして、$\rho_{01}$ と $\rho_{02}$ の減衰を密度行列から直接読み取り、比が $1:4$ になっていることを確かめよ。
