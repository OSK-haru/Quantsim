...
4. 🔬 **回路スタジオ**で「Bell状態にリセット」を押し、**シミュレーションワークスペース**へ移動して実行せよ。実行後に**状態エクスプローラー**を開き、「表示項目」から「密度行列」と「出力確率」を表示する。「理想状態の密度行列」で $\rho_{00,00}$、$\rho_{11,11}$ と非対角の $\rho_{00,11}$、$\rho_{11,00}$ がそれぞれ約0.5になること、出力確率の $|00\rangle$ と $|11\rangle$ が対角成分と一致することを確かめよ。

5. 🔬 同じBell回路のまま、**シミュレーションワークスペース**の「計算パラメーター・測定・スナップショット」→「デバイス品質」を既定値から下げて再実行せよ。**状態エクスプローラー**の「Gate-aware 密度行列」を右側の「理想状態の密度行列」と比べ、$\rho_{00,11}$ と $\rho_{11,00}$ の絶対値がどう変わるかを観察する。「指標タイムライン」の最終純度も記録し、1との差を求めよ。
