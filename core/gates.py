...
def apply_gate_operation(rho: Matrix, gate: GateOperation, n_qubits: int) -> Matrix:
    ...
    if gate_type == "CNOT":
        ...
        unitary = expand_cnot(gate.controls[0], gate.targets[0], n_qubits)
        return apply_unitary_to_density(rho, unitary)
    ...
    unitary = expand_single_qubit_gate(one_qubit_gate, gate.targets[0], n_qubits)
    return apply_unitary_to_density(rho, unitary)
