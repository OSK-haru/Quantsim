...
contract_versions: pulse-baseline-a-v1 | pulse-extension-b-v1
model_ids:
...
estimated above 200,000 total internal steps; the qutrit path uses the stricter
4,000-step ceiling described below. A timeout response states that previous
...
The qutrit HTTP work ceiling is 4,000 internal RK4 steps. This is stricter
than the 25,000-step core validation ceiling because the API has a 15-second
wait timeout. Requests over the HTTP ceiling receive an actionable `422`
