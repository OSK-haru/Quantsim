...
formal_execution: implementation freeze後
```

本書はVALIDATION-8の監査方針を先に固定するための計画書である。
現時点では実機データを取得せず、正式な合否判定も行わない。
...
プロバイダーは本計画書では固定しない。正式実行時に、利用可能性、利用規約、
費用、回路機能、データ取得可能性を再確認して選定する。
...
```text
Protocol definition                 <- current stage
  |
...
- [ ] Pulse Baseline Aが完了している
- [ ] Pulse Extension Bが完了している
- [ ] Rust本番候補経路が実装されている
- [ ] Python/Rust parityが確認されている
- [ ] CPTP-preserving経路が確認されている
...
- [ ] calibration subsetとholdout subsetが固定されている
...
4. 正式実行はPulse A/B、Rust parity、CPTP-preserving経路、全回帰後とする。
5. 現時点ではプロトコルのみを追加し、実機jobは送信しない。
6. Pulse-level実機監査はV8から分離する。
