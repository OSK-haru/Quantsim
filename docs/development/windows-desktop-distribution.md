...
Produce a Windows submission build that opens QuantaScope by double-clicking
`QuantaScope.exe`, starts no visible command prompt, and does not require Node,
...
Uvicorn and a small Windows GUI launcher named `QuantaScope.exe`. The GUI
...
```text
release/windows/QuantaScopeBackend/QuantaScope.exe
release/windows/QuantaScopeBackend/QuantaScope.lnk
```

Reviewers can open `QuantaScope.exe` or `QuantaScope.lnk`. Both start the
hidden backend and open the default browser.

## Submission checks

- Test on a clean Windows account or virtual machine.
- Confirm that `QuantaScope.exe` opens the browser without a terminal window.
...
```text
%LOCALAPPDATA%\QuantaScope\launcher.log
```
